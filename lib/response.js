(function (root, factory) {
    ...
    Response = Class.extend({
        ...
        init: function (req) {
            ...
            this._scrollOrAnimateTop=function(y,delay){
              if ($.device.touch) {
                this._scrollTop(0,delay);
              }else{
                var self=this;
                setTimeout(function(){
                  self._animateTop(y);
            ...
            this._scrollTop=function(y,delay){
              setTimeout(function(){
                window.scrollTo(0,y);
              },delay);
            };

            this._animateTop=function(y){
              $('html,body').animate({
                scrollTop: y
              }, 500);
            };
            ...
            this.scrollTop=function(delay){
              ...
              if(typeof delay==='object'){
                var obj=delay;
                var delayValue=100;
                var yOffset=obj.yOffset;
                if(obj.y) y=obj.y;
                if(obj.delay) delayValue=obj.delay;
                if(yOffset){
                  var currentYOffset=window.pageYOffset;
                  if(currentYOffset > yOffset){
                    this._scrollTop(y,delayValue);
                  }
                }else{
                  this._scrollTop(y,delayValue);
                }
              }else{
                this._scrollTop(y,delay);
              }
            };
            ...
            this.animateScrollTop=function (delay) {
              ...
              if(typeof delay==='object'){
                ...
                if(yOffset){
                  var currentYOffset=window.pageYOffset;
                  if(currentYOffset > yOffset){
                    this._scrollOrAnimateTop(y,delayValue);
                  }
                }else{
                  this._scrollOrAnimateTop(y,delayValue);
                }
              }else{
                this._scrollOrAnimateTop(y,delay);
              }
            };
